...
var newEntrys = [];
  ...
  for (var i = 0; i < Entrys.length; i++){
    ...
    if(date >= start && date <= end && contains){
          //insert_row(Entrys[i]);
          newEntrys.push(Entrys[i]);
      }
    }
    newEntrys.sort(function(e1,e2){
    ...
    if(d1 <d2){
      ...
      for (var i = 0; i < newEntrys.length; i++){
        insert_row(newEntrys[i]);
        //console.log(newEntrys[i]);
       }
    }
}

var work_hours_flag = false;
function sort_by_work_hours(){
